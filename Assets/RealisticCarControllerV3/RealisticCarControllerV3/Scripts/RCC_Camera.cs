...
public class RCC_Camera : MonoBehaviour
{
   ...
   // private VehicleNitro playerNitro;
    ...
    void OnEnable()
    {
        ...
        OnBCGCameraSpawned?.Invoke(gameObject);
    ...
    {
        //Debug.Log("SetTarget.." + player.name);
    ...
    void Update()
    {
        ...
        //if (playerCar && !playerNitro)
        //	playerNitro = playerCar.GetComponent<VehicleNitro> ();
        ...
        /*if (playerNitro != null && playerNitro.NOSBool)
        {
            // t += Time.deltaTime;
            // Debug.Log (t);

            thisCam.fieldOfView = Mathf.Lerp(thisCam.fieldOfView, 100f, Time.deltaTime * 10f);
        }
        else
    ...
    void TPS()
    {
        ...
        pivot.transform.localPosition = Vector3.Lerp(pivot.transform.localPosition, newPivotPos, (isPivotRotationTimescaleindependent) ? Time.unscaledDeltaTime * pivotRotationSpeed : Time.deltaTime * pivotRotationSpeed);
        pivot.transform.localRotation = Quaternion.Lerp(pivot.transform.localRotation, newPivotRotation, (isPivotRotationTimescaleindependent) ? Time.unscaledDeltaTime * pivotRotationSpeed : Time.deltaTime * pivotRotationSpeed);
    ...
    Vector3 newPivotPos = Vector3.zero;
    Quaternion newPivotRotation = Quaternion.Euler(Vector3.zero);
    float pivotRotationSpeed = 1f;
    bool isPivotRotationTimescaleindependent = true;
    public void SwitchPivotAngle(Vector3 positionToReach, Quaternion rotationToReach, float pivotRotSpeed = 1f, bool timescaleIndependent = false)
    {
        newPivotPos = positionToReach;
        newPivotRotation = rotationToReach;
        pivotRotationSpeed = pivotRotSpeed;
        isPivotRotationTimescaleindependent = timescaleIndependent;
