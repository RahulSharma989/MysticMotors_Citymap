...
public class RCC_DashboardInputs : MonoBehaviour {
	...
	[SerializeField] Button _driftButton;
	...
	void GetValues(){
		...
		CheckForBoostPressed();
		CheckForDriftPressed();
		if(RPMNeedle){
    ...
    private void CheckForDriftPressed()
    {
        if (Input.GetButton("Jump"))
        {
			//Debug.LogError("Interactable");
			_driftButton.interactable = false;
        }
        else
        {
			_driftButton.interactable = true;
		}
    }

    private void CheckForBoostPressed()
    {
