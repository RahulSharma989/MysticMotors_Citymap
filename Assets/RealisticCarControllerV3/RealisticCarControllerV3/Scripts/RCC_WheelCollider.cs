...
public class RCC_WheelCollider : MonoBehaviour
{
   ...
   // public PowerUpsHandler powerUpsHandler = null;
    ...
    [SerializeField] private RCC_Skidmarks skidmarks;        // Main Skidmarks Manager class.
    [SerializeField] private float startSlipValue = .25f;        // Draw skidmarks when forward or sideways slip is bigger than this value.
    [SerializeField] private int lastSkidmark = -1;
    ...
    public AudioMixerGroup musicAudioMixer = null;
    ...
    void Start()
    {
     ...
     //   powerUpsHandler = GetComponentInParent<PowerUpsHandler>();
        carController = GetComponentInParent<RCC_CarControllerV3>();
        rigid = carController.GetComponent<Rigidbody>();
        musicAudioMixer = carController.audioMixer;
        ...
        switch (RCCSettings.behaviorType)
        {
            ...
            case RCC_Settings.BehaviorType.SemiArcade:
             //   Debug.Log("Behaviour type " + RCCSettings.behaviorType);
        ...
        if (RCCSettings.useSharedAudioSources)
        {
            ...
            if (!carController.transform.Find("All Audio Sources/Skid Sound AudioSource"))
                audioSource = RCC_CreateAudioSource.NewAudioSource(carController.gameObject, "Skid Sound AudioSource", 5f, 50f, 0f, audioClip, true, true, false, musicAudioMixer);
            else
    ...
    void Update()
    {
        ...
        {
            SkidMarks();
            //RCCSettings.behaviorType = RCC_Settings.BehaviorType.Drift;
        }
        if (_isAI)
        {
            SkidMarks();
            Debug.Log("Enabled Skidmarks");
        }
        if (_shouldSkid)
        {
            SkidMarks();
        ...
        if (!carController.isSleeping)
        {
            ...
            /*if (powerUpsHandler && !powerUpsHandler.isAI)
            {
                for (int i = 0; i < allWheelParticles.Count; i++)
                    allWheelParticles[i].gameObject.SetActive(powerUpsHandler.IsCar());
            }*/
        }
    ...
    public void WheelAlign()
    {
        ...
        wheelModel.position += ((compression - 1.0f) * WheelCollider.suspensionDistance * transform.up);
    ...
    public void SkidMarks()
    {
        ...
        if (skidmarks)
        {
            /*if (powerUpsHandler && (!powerUpsHandler.isAI) && !powerUpsHandler.IsCar())
                return;*/
            // If slips are bigger than target value...
            if (wheelSlipAmountSideways + wheelSlipAmountForward > startSlipValue)
            {
                Vector3 skidPoint = GroundHit.point + 2f * Time.deltaTime * (rigid.velocity);
    ...
    /*void SendUpdatedSkidList()
    {
        object[] _data = new object[] { Photon.Pun.PhotonNetwork.LocalPlayer.ActorNumber, skidmarks.skidmarks };
        PhotonManager.PhotonRaiseEventsSender_Other(PhotonManager.AddSkid, _data);
    }*/
    ...
    void Audio()
    {
        ...
        if (totalSlip > startSlipValue)
        {
            ...
            if (rigid.velocity.magnitude > 1f)
            {
                ...
                audioSource.volume = Mathf.Lerp(0f, .12f, totalSlip - 0);
